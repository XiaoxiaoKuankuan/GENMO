...
首次正式训练与短程验收使用不同目录和初始化来源。

**当前正式规模：每卡batch 256、8卡全局2048、累计1、350,000个优化步。**
前缀采用85% P∈[6,18]+15% P=0；首版部署约定P=12。正式命令仅供用户执行。
本次新batch真实GPU验收已完成，使用完整模型、真实四库、新前缀分布和正式warmup500：

| 当前batch256验收 | 实际步数 | loss范围 | 裁剪前梯度norm范围 | 峰值allocated / reserved GiB | 首步后平均秒/步 |
|---|---:|---:|---:|---:|---:|
| 8卡首次训练 | 0→10，共10步 | 0.43047～0.50069 | 1.77432～2.25879 | 31.575 / 32.357 | 0.77572 |
| 同checkpoint完整恢复 | 10→15，共5步 | 0.40532～0.44103 | 1.64689～1.88557 | 31.571 / 32.092 | 0.68521 |

两段退出码均0、8个rank最终权重哈希各自一致，loss/梯度有限。直接读取checkpoint确认
Adam与scheduler步数10→15，各rank采样游标2560→3840，恢复了optimizer/scheduler/RNG。
BF16 scaler明确禁用。每段四库各2个固定val样本做纯条件DDIM20/CFG2.5，合计16次，
覆盖P=0和非零前缀；输出shape正确、有限，最终physical及每步normalized已知坐标误差0。
只覆盖max_steps=10/15与log_every_steps=1，其他正式参数不变。机器可读证据见
[每卡256验收报告](stage5_server1_b256_acceptance.json)，实测代码为119883b。

首步分别3.497/3.462秒；含段尾验证与保存的循环总耗时16.370/12.795秒。step计时包含
取batch/前反传/更新，不含步后日志、验证/checkpoint和进程启动，两段都未跨采样epoch，
不以15步推断完整35万步工期或收敛。GPU短训结束后临时目录已精确清理，17文件共
5,160,104,693 bytes，八卡0MiB/0%，无训练进程或tmux会话，正式实验和控制台目录均未创建。
服务器已核验tmux3.2a、TensorBoard2.21.0；6006端口检查时空闲。以下保留旧小batch历史验收。

**2026-09-23 真实 GPU 短程验收通过：单卡 10 步 → 8 卡 DDP 10 步 → 完整恢复再 5 步。**
使用四库真实数据和完整原音乐权重初始化模型，没有使用缩小网络。每卡 batch 32、
梯度累计 1、有效全局 batch 256 已实际通过 8 卡训练与恢复；当时拟定的50,000步
没有执行，现已按用户要求改为每卡256/350,000步。历史机器可读证据见[验收报告](stage5_server1_acceptance.json)。

**前缀策略后续更新：** 以下GPU验收发生在旧P∈[0,24]配置下；随后按用户要求将正式训练
改为15% P=0、85% P∈[6,18]。旧验收报告保留原始事实，不冒充新分布的GPU训练结果。
...
不含启动、数据索引与权重加载，不能据此承诺正式训练工期或更大 batch 的吞吐。
...
前缀以15%概率取P=0，其余85%在6～18帧的整数区间均匀取值；动作布局始终为120帧 @ 30 Hz。
没有改成 51D，没有增加 joint velocity
预测，没有接入 GMT、Critic 或 DPPO，也没有修改 30→50 Hz 速度派生链。

12帧是非零训练区间的中心/典型值，不额外增加某一个长度的抽样权重。正式配置中的
`sample_contract.prefix_min_frames/max_frames/zero_probability` 控制此混合分布；各来源共用
相同概率，四库20/35/25/20权重不受影响。15%是请求P=0的名义概率；尾部样本仍按真实
剩余长度裁短，effective P可能小于6或变为0，已有meta同时记录requested/effective值。

首版部署约定记录在 `deployment_reference.prefix_frames: 12`，按30Hz帧周期计约0.4秒，
以后根据实际端到端P95延迟调整。此字段记录调用策略，不会自动改写GMT或启动在线发布器；
接入时由调用者构造12帧前缀及逐坐标mask，Actor仍支持可变P和P=0。

此前新策略先在服务器1完成CPU只读数据验收：四库各64个真实窗口，共256个全部通过既有batch
validator和逐坐标前缀检查，29个P=0、4个尾部裁短；未创建模型或执行训练。每来源10万
稳定键分布探针的P=0比例为14.744%～15.224%。具体测试及一次CPU-DDP退出超时后独立
复跑通过的记录见仓库`记录文本.md`。
...
| 正式输出目录 | `/data0/user/liwei/GENMO_outputs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1` |
...
以下规模按用户要求恢复为此前每卡256、350,000步；初始化仍为原音乐weights-only。
...
| 每卡 batch | 256 |
| 梯度累计 | 1 |
| 有效全局 batch | `8 × 256 × 1 = 2048` |
| 训练终点 | 350,000 个 optimizer 更新步 |
| 精度 | BF16；物理损失内部 FP32 |
| AdamW 学习率 | `1e-5` |
| weight decay | `0.01` |
| 梯度裁剪 norm | `1.0` |
| scheduler | 前 500 步 warmup，固定 350,000 步 cosine，末端比例 `0.1` |
...
本次batch256短程测试保持正式scheduler总周期350,000和warmup500。完整恢复要求保持采样、精度和
...
```bash
ssh -p 50030 user@112.65.216.193
```

在服务器1新建训练tmux会话：

```bash
tmux new-session -s bumi-cl-s1-b256-s350k -n train
```

进入tmux后再执行下面的训练命令。按`Ctrl+b`后松开，再按`d`可离开会话并保持训练运行；
重新进入使用`tmux attach -t bumi-cl-s1-b256-s350k`。本轮只交付这些命令，未创建tmux会话。
...
mkdir -p /data0/user/liwei/training_logs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1
/home/user/liwei/GENMO/.venv/bin/python -u -B -m torch.distributed.run \
  ...
  --output-dir /data0/user/liwei/GENMO_outputs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1 \
  2>&1 | tee -a /data0/user/liwei/training_logs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1/train.log
```

训练命令在tmux窗口内前台执行，不包含自动重启。控制台日志写到独立的training_logs目录，
实验目录由训练器创建；不要预先把日志写入正式输出目录内部，以免入口判定新目录非空。
...
experiment = Path('/data0/user/liwei/GENMO_outputs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1').resolve()
...
初始化权重的 lineage。训练已达到 step 350,000 时无需再次恢复；入口要求新运行终点
严格大于恢复步数。入口拒绝回退覆盖更晚的完整 checkpoint；若恢复后在下次保存前再次
中断，可重复同一恢复命令，新尝试使用独立 attempt 后缀，保留前次配置、日志和验证报告。

## 日志、checkpoint 与验证结果

查看控制台日志（含初始化报错和每10步训练摘要）：

```bash
tail -n 50 -f /data0/user/liwei/training_logs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1/train.log
```

查看每一步的完整训练记录：

```bash
tail -n 20 -f /data0/user/liwei/GENMO_outputs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1/train_metrics.jsonl
```
...
在服务器1另一个SSH终端新建TensorBoard会话：

```bash
tmux new-session -s bumi-cl-s1-tb -n tensorboard
```

进入该tmux后运行TensorBoard，随后可按`Ctrl+b`、`d`离开：

```bash
/home/user/liwei/GENMO/.venv/bin/python -B -m tensorboard.main \
  --logdir /data0/user/liwei/GENMO_outputs/bumi_closedloop_stage1_90505_p6to18_z15_b256_s350k_8gpu_v1/tensorboard \
...
```bash
ssh -p 50030 -N -o ExitOnForwardFailure=yes -L 16006:127.0.0.1:6006 user@112.65.216.193
```

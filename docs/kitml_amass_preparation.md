...
代码构建提交 `ee2c24747ba52613c0c21851935cf88da284da47`。单元测试17项通过，
服务器全部产物重载复核通过；原始ZIP的15,644个成员CRC全部通过。

| 最终状态 | 动作数 | 原始文本数 |
|---|---:|---:|
| ready | 2,884 | 6,079 |
| no_text | 899 | 0 |
| unmapped（有文本但无对应映射） | 124 | 263 |
| missing_amass | 2 | 6 |
| alignment_mismatch | 2 | 5 |
| 合计 | 3,911 | 6,353 |

原Unifier未匹配210条中的86条同时无文本，最终互斥状态归入no_text；因此本表unmapped为124。
可用动作占73.7407%，可用文本占95.6871%。可用来源KIT 2,049、CMU 793、EKUT 42；
总688,026帧、6.370611小时。2,091条可用动作从推导出的100/3Hz重新采样到30Hz。
只通过名义30Hz解释无法获得这些正确时长，因此保留了逐条时钟推导证据。

- 原KIT ZIP：4,159,992,190字节；展开内容应为8,658,554,990字节，当前直接读ZIP。
- 已传入的AMASS容器：1,979,394,441字节；两端SHA一致。
- 2,884个NPZ：173,456,465字节；含两份metadata的完整genmo_30hz目录193,615,703字节。
- 全部ID及6,353条原始caption逐项核对一致；所有输出的姿态/平移/形状有限，模型身份为smplx。
- 独立线性时间采样复核最大平移误差1.1921e-7米；504,790个源/目标重合时刻的关节旋转
  复核最大误差1.6363e-7弧度；目标时长相对MMM最大量化差1/60秒。
- 两条缺文件：01206、01207，KIT/572/dance_waltz 01、02。
- 两条错配：01662（EKUT/300/Tpose）与02212（KIT/291/drum05）；后者与00661重复指向
  同一源文件但原MMM帧数不同，不能按文件名合并文本。

报告：`summary.json`、`not_ready.json`、`download_integrity.json`、`verification.json`、
`metadata_example.json`，均位于上述服务器报告目录。下载分片4,821,459,057字节在完整
ZIP验收后精确清除，原始ZIP、原始容器和正式metadata/NPZ保留。

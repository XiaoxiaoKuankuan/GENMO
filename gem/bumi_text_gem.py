...
class BumiTextGEM(BumiGEM):
    ...
    def __init__(self, *args, **kwargs):
        ...
        expected_kind = (
            "bumi_text_crop120"
            if self.sequence_contract["schema_version"] == 2
            else "bumi_text_fullseq"
        )
    ...
    def on_fit_start(self):
        ...
        expected = (
            set(self.stats_identity.get("datasets", ["motionmillion", "humanml3d"]))
            if selection is None
            else {selection}
        )
        if {identity["dataset"] for identity in identities} != expected:
            raise ValueError("联合和单集实验必须采用对应train统计量")
        if self.sequence_contract["schema_version"] == 2 and any(
            identity.get("sequence_mode") != "crop" or identity.get("pad_to_frames") != 120
            for identity in identities
        ):
            raise ValueError("crop120模型不能读取full300训练Dataset")
    ...
    def create_condition_mask(
        ...
        if first_k_frames is not None and first_k_frames != batch["L"]:
            raise ValueError("已选定的文本窗口禁止再次 first_k_frames 裁剪")
                ...
                **(
                    {
                        "sampling": OmegaConf.to_container(
                            self.model_cfg.sampling_contract, resolve=True
                        )
                    }
                    if self.sequence_contract["schema_version"] == 2
                    else {}
    ...
    def load_pretrained_model(self, ckpt_path):
        raise ValueError(
            "首版BUMI文本不提供weights-only初始化；从零训练或使用同契约resume_mode完整恢复"
        )

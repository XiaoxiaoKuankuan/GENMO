...
from gem.diffusion_utils.losses import discretized_gaussian_log_likelihood, normal_kl
from gem.diffusion_utils.nn import mean_flat, sum_flat
...
class GaussianDiffusion:
    ...
    def p_mean_variance_guided(
        ...
        if guide is not None:
            model_output = guide.guide(x, model_output, target_motion, model_variance, t)
    ...
    def ddim_sample(
        ...
        model_kwargs["y"]["pred_x_start"] = out_orig["pred_xstart"]
    ...
    def ddim_get_xt(self, x, t, pred_xstart, eta):
        ...
        # Equation 12.
        mean_pred = (
            pred_xstart * th.sqrt(alpha_bar_prev) + th.sqrt(1 - alpha_bar_prev - sigma**2) * eps
        )
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
        )  # no noise when t == 0
    ...
    def plms_sample(
        ...
        if order > 1 and old_out is None:
            ...
            old_eps = [eps]
            mean_pred = (
                out["pred_xstart"] * th.sqrt(alpha_bar_prev) + th.sqrt(1 - alpha_bar_prev) * eps
            )
            eps_2, _, _ = get_model_output(mean_pred, t - 1)
            eps_prime = (eps + eps_2) / 2
            pred_prime = self._predict_xstart_from_eps(x, t, eps_prime)
            mean_pred = (
                pred_prime * th.sqrt(alpha_bar_prev) + th.sqrt(1 - alpha_bar_prev) * eps_prime
            )
        else:
            ...
            if cur_order == 1:
                ...
                eps_prime = (
                    55 * old_eps[-1] - 59 * old_eps[-2] + 37 * old_eps[-3] - 9 * old_eps[-4]
                ) / 24
            else:

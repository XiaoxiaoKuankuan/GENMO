...
class BumiPipeline(nn.Module):
    ...
    def __init__(self, args, args_denoiser3d, **_kwargs: Any) -> None:
        ...
        if train_modes != ["diffusion"]:
            raise ValueError(
                f"BumiPipeline supports only train_modes=['diffusion'], got {train_modes}"
            )
        if list(args.get("in_attr", ())) not in ([], ["encoded_music"]):
            raise ValueError("BumiPipeline accepts only in_attr=['encoded_music']")
        ...
        if not isinstance(self.endecoder, BumiEndecoder):
            raise TypeError("BumiPipeline endecoder_opt must instantiate BumiEndecoder")
    ...
    def forward(
        ...
        if self.args.get("loss_reduction") == "valid_per_sample":
            ...
            indices = torch.arange(pred_x.shape[1], device=pred_x.device)[None].expand(
                pred_x.shape[0], -1
            )

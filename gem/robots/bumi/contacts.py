...
def derive_bumi_foot_contact(
    ...
    *,
    ...
    if valid_length_mode:
        # 真实末帧沿用最后一对真实帧速度，不能取真实帧→重复 padding 的零速度。
        lengths = valid.sum(-1)
        last_speed = speed.gather(-2, (lengths - 2).clamp_min(0)[..., None, None].expand(*lengths.shape, 1, 2))
        frame = torch.arange(speed.shape[-2], device=speed.device)
        speed = torch.where((frame >= lengths[..., None] - 1)[..., None], last_speed, speed)

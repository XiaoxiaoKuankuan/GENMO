...
倾斜包络的异常大倾角。文本完整动作保留GT中的合法躺卧，没有统一直立目标。
...
# 上下文变量只影响显式 fullseq 调用，线程/嵌套调用互不污染；旧音乐标量归约不变。
_PER_SAMPLE = ContextVar("bumi_valid_per_sample", default=False)
...
def _masked_mean(value: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    if _PER_SAMPLE.get():
        return torch.where(expanded, value, 0).float().flatten(1).sum(1) / expanded.flatten(1).sum(1).clamp_min(1)
...
class BumiRobotLosses(nn.Module):
    ...
    def __init__(
        ...
        loss_reduction: str = "legacy",
    ) -> None:
        super().__init__()
        self.endecoder = endecoder
        if loss_reduction not in {"legacy", "valid_per_sample"}:
            raise ValueError("未知 BUMI loss reduction")
    ...
    def forward(self, inputs, model_output, decode_dict, pred_qpos_canonical, pred_fk, *, global_step=0):
        """保留逐样本损失到最后再应用扩散权重，不让长动作获得额外 batch 权重。"""
        token = _PER_SAMPLE.set(self.loss_reduction == "valid_per_sample")
        try:
            result = self._forward_losses(inputs, model_output, decode_dict, pred_qpos_canonical, pred_fk, global_step=global_step)
        finally:
            _PER_SAMPLE.reset(token)
        if self.loss_reduction == "valid_per_sample":
            batch_size = pred_qpos_canonical.shape[0]
            weights = model_output.get("t_weights", pred_qpos_canonical.new_ones(batch_size))
            if weights.shape != (batch_size,) or not torch.isfinite(weights).all():
                raise ValueError("t_weights 必须是有限 [B]")
            result = {key: ((value * weights).mean() if key == "loss" or key.startswith("weighted_") else value.mean())
                      if value.ndim == 1 else value for key, value in result.items()}
        return result
    ...
    def _forward_losses(
        ...
        *,
        ...
        root_mask = valid
        if self.loss_reduction == "valid_per_sample":
            root_mask = valid[..., None].expand(*valid.shape, 3).clone()
            # XY 是向前差分，最后真实帧没有下一帧；Z 是逐帧高度，仍保留监督。
            pairs = torch.cat((valid[:, :-1] & valid[:, 1:], torch.zeros_like(valid[:, :1])), dim=1)
            root_mask[..., :2] &= pairs[..., None]
        raw["repr_root_pos"] = _masked_mean(
            (pred_norm[..., :3] - target_norm[..., :3]).square(), root_mask
        )

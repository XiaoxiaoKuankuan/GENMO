...
可配置维度、两窗 120/30 拼接时的世界根连续性、qpos 二进制包 CRC/revision/绝对帧号、
跨分块速度与 XML 关节限位安全门，以及增量 30→50 Hz 计划和离线重采样的一致性。
...
def test_bumi_sliding_reencodes_overlap_and_preserves_world_root(
    ...
    np.testing.assert_allclose(
        generated.qpos[:, 0].numpy(), np.arange(210) / 30.0, atol=1.0e-5
    )
...
def test_bumi_incremental_plan_matches_offline_30_to_50() -> None:
    ...
    expected_frames = qpos_timeline_to_gmt_frames(
        final.qpos, fps=50.0, native_to_gmt=np.arange(21)
    )

...
自包含网页目录优先硬链接且拒绝静默覆盖不同目标的规则。完整 40 项媒体和轨迹由正式运行
后的逐文件验证负责。
"""
...
def test_materialize_file_prefers_hardlink_and_reuses_identity(tmp_path: Path) -> None:
    ...
    with pytest.raises(FileExistsError, match="身份不同"):
        materialize_file(source, target)

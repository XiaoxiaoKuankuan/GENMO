...
class GMRUDPBridgeTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.bridge = GMRUDPBridge(
            "127.0.0.1", self.receiver.getsockname()[1]
        )
    ...
    def test_packet_layout_axis_conversion_and_quaternion(self) -> None:
        ...
        self.assertAlmostEqual(np.linalg.norm(chest[3:]), 1.0, places=6)
    ...
    def test_initial_yaw_is_cancelled(self) -> None:
        ...
        # C @ R_ay == Rz(yaw), so normalization should output identity.
        rotation_ay = torch.from_numpy(axis_convert.T @ rz)

...
"""保留 BUMI 音乐源数据审核所需的离线网格渲染函数。

本模块从退役的 SMPL Demo 工具中原样提取 render_global_frames，供 AIOZ、
FineDance、CoMPAS3D 的数据验证器使用。相机、颜色、地面与逐帧渲染算法不变；
本模块不加载生成模型、文本编码器或 Webcam，Open3D 仍只在函数内部延迟导入。
"""
